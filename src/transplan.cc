...
namespace TRANSPLAN
{
    ...
    {
        std::cout << "Number of Activities      = " << Transplan::n_activities << std::endl
                << "Number of State Variables = " << Transplan::n_statevars << std::endl
                << "Number of Resources       = " << Transplan::n_resources << std::endl
                << "Number of SV Transitions  = " << Transplan::n_sv_trans << std::endl
                << "Number of Res Transitions = " << Transplan::n_r_trans << std::endl
                << "Number of Sq Matrices     = " << Transplan::n_sq_matrices << std::endl;
    ...
    int Transplan::createNewResource(std::string name,
            int cap,
            RES_TYPE type = TRANSPLAN::REUSE_R)
    {
     ...
     */
    int Transplan::createNewStateVariableTransition(int sv_index,
            int a_index,
            int from,
            int to,
            int dur,
     ...
     */
    int Transplan::createNewResourceTransition(int r_index,
            int a_index,
            int req,
            TRANS_TYPE type,
            int dur,
            ...
            if (SV(sv_idx).goal_state_index == -1)
            {
                ...
                while (itr != SV(sv_idx).states.end())
                {
                    if (itr->name != "S" && itr->name != "G" && itr->name != "IG" && !itr->is_non_goal_state)
                    {
                        ...
                        int dummy_id = this->createNewStateVariableTransition(sv_idx,
                                act_idx,
                                from,
                                to,
                                0,
        ...
        for (int r = 0; r < n_resources; ++r)
        {
            if (RES_TYPE(r) == TRANSPLAN::RESERV_R)
            {
                ...
                if (init_level > 0)
                {
                    ...
                    int dummy_id = this->createNewResourceTransition(r,
                            act_idx,
                            init_level,
                            TRANSPLAN::PROD,
                            0,
    ...
    void Transplan::add_DUMMY_S_E()
    {
        ...
        for (int i = 0; i < Transplan::n_statevars; ++i)
        {
            ...
            int dummy_s = this->createNewStateVariableTransition(SV(i).index,
                    DUMMY_START,
                    s_from,
                    s_to,
                    0,
                    0,
                    SV(i).init_setup_state);
            ...
            int dummy_e = this->createNewStateVariableTransition(SV(i).index,
                    DUMMY_END,
                    e_from,
                    e_to,
                    0,
                    0,
                    SV(i).goal_setup_state);
        ...
        for (int i = 0; i < Transplan::n_resources; ++i)
        {
            ...
            int dummy_s = this->createNewResourceTransition(RES(i).index,
                    DUMMY_START,
                    req,
                    TRANSPLAN::BRR,
                    0,
                    0,
                    RES(i).init_setup_state,
                    0);
            ...
            int dummy_e = this->createNewResourceTransition(RES(i).index,
                    DUMMY_END,
                    req,
                    TRANSPLAN::BRR,
                    0,
